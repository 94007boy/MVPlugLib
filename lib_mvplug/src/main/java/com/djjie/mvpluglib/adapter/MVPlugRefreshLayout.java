...
public class MVPlugRefreshLayout extends ViewGroup {
    private static final String Logger_TAG = "CustomeSwipeRefreshLayout";
    public static final int HEADER_VIEW_HEIGHT = 70;// HeaderView height (dp)
    ...
    public MVPlugRefreshLayout(Context context) {
        this(context, null);
    }

    @SuppressWarnings("deprecation")
    public MVPlugRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                Logger.d(Logger_TAG, "debug:onTouchEvent ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
