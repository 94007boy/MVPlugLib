...
public class DemoPresenter extends DemoContract.Presenter {
    ...
    void getDemoList(int state) {
        subscribe(state,repository.getDemoList(state),new ResRecyclerObserver<DemoRes>(this,state){
            @Override
            public void onResult(DemoRes demoRes, int state) {
                if (demoRes != null && demoRes.getDataShows() != null && !demoRes.getDataShows().isEmpty()){
                    ...
                    repository.setPageFlagByTag("list",pageFlag);//记录分页标示
                }
                MVPlugAdapterPresenter adapterPresenter = getView().viewBinding.demoRecyclerView.getPresenter();
